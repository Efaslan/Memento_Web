...
export default function AddMedicationSchedule({ patientId, onClose, onSuccess, editData }: AddMedicationScheduleProps) {
  ...
  const [timeInputs, setTimeInputs] = useState<string[]>(
    editData && editData.times && editData.times.length > 0 
      ? editData.times.map(t => t.time.substring(0, 5)) 
      : ['']
  );
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      const payload: MedicationScheduleRequestDto = {
        ...
        endDate,
        isPrn,
        // Eğer PRN seçiliyse zamanları sıfırlayıp boş liste gönderiyoruz
        times: isPrn ? [] : validTimes 
      };
  ...
  const containerClasses = "fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-[400px] bg-white rounded-2xl shadow-2xl border border-slate-200 z-50 animate-in fade-in zoom-in-95 duration-200 text-left max-h-[90vh] overflow-y-auto custom-scrollbar";

  return (
    <>
      {/* backdrop-blur-sm KALDIRILDI. Sadece hafif şeffaf siyah zemin kaldı */}
      ...
      <div className={containerClasses}>
        ...
        <div className="p-5 relative">
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            ...
            {/* Sadece isPrn FALSE ise Kullanım Saatleri görünür */}
            {!isPrn && (
              <div className="border border-slate-100 bg-slate-50 p-3 rounded-xl mt-2 animate-in fade-in slide-in-from-top-2 duration-200">
                <div className="flex justify-between items-center mb-3">
                  <label className="text-sm font-medium text-slate-700">Kullanım Saatleri</label>
                  <button type="button" onClick={addTimeInput} className="text-xs font-bold text-indigo-600 hover:text-indigo-700 bg-indigo-100 px-2 py-1 rounded transition-colors">
                    + Saat Ekle
                  </button>
                </div>
                
                {/* Alt alta dizilim yerine grid ile 2 sütunlu yan yana dizilim */}
                    ...
                    <div key={index} className="flex items-end gap-1">
                      ...
                      {timeInputs.length > 1 && (
                        <button 
                          ...
                          className="mb-5.5 p-1.5 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors"
                          title="Saati Sil"
                        >
